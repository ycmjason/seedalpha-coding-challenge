...
function start(args, done){
  var start_of_yesterday = pureDate(new Date(Date.now() - 24 * ONE_HOUR));
  var wait = Wait(24, done);
  for(let i = 0; i < 24; i++){
    findEmailsWithDateAndHour(start_of_yesterday, i, function(err, emails){
      if(err) throw err;
